...
Deno.serve(async (req) => {
  ...
  if (!visitor_id || !case_id || !argument_key || !vote) return new Response(JSON.stringify({ error: "Missing fields" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  ...
  const { data: allVotes } = await supabase
    ...
    .select("vote")
    .eq("case_id", case_id)
    .eq("argument_key", argument_key);

  let up_count = 0, down_count = 0;
  if (allVotes) allVotes.forEach((v: any) => { if (v.vote === "up") up_count++; else down_count++; });

  return new Response(JSON.stringify({ ok: true, argument_key, up_count, down_count, my_vote: vote }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
});
